                ...
                <I18n ns="translations">
                    {
                        (t, {i18n}) => (
                            <div className="widget">
                                <div>
                                    <img className="songImg" src={this.props.songImg}/>
                                    <p><b>{t('song')}: </b><i>{this.props.songName}</i></p>
                                    <p><b>{t('artist')}: </b><i>{this.props.songArtist}</i></p>
                                </div>
                                <div>
                                    <button className="sp-button"
                                            onClick={() => this.props.prevSong(this.props.authToken)}>{t('previoussong')}</button>
                                    <button className="sp-button"
                                            onClick={() => this.props.saveSong(this.props.authToken, this.state.songId)}>{t('savesong')}</button>
                                    <button className="sp-button"
                                            onClick={() => this.props.nextSong(this.props.authToken)}>{t('nextsong')}</button>
                                </div>
                                <div className="options">
                                    <label>{t('shuffle')}<input
                                        onClick={() => this.props.shuffleCheck(this.props.authToken, !this.state.musicShuffle)}
                                        id="shuffle" type="checkbox" placeholder="Shuffle"/></label>
                                </div>
                            </div>
                        )
                    }
